"""Flow class for CICFlowMeter-style feature extraction."""
...
class Flow:
    ...
    def __init__(self, packet: Packet, direction: PacketDirection):
        """Initialize a Flow object from the first packet.

        Args:
            packet: A packet from the network.
            direction: The direction the packet is going over the wire.
        """
        (
            self.src_ip,
            self.dest_ip,
        ...
        self.packets: List[tuple] = [(packet, direction)]  # Add the first packet
        self.flow_interarrival_time: List[float] = []
        self.start_timestamp = packet.time
        self.latest_timestamp = packet.time  # Initialize latest_timestamp too
        self.protocol = getattr(packet, 'proto', 0)
        if hasattr(packet, 'proto'):
            try:
                self.protocol = int(packet.proto)
            except:
                self.protocol = 0
    ...
    def get_data(self, include_fields: Optional[List[str]] = None) -> Dict[str, Any]:
        """Obtain the values of the features extracted from each flow.

        Args:
            include_fields: Optional list of field names to include (if None, include all)
        ...
        # Map to CICIDS feature names (for compatibility with features.pkl)
        cicids_mapping = {
            "Destination Port": float(self.dest_port),
            "Total Length of Fwd Packets": data["totlen_fwd_pkts"],
            "Fwd Packet Length Max": data["fwd_pkt_len_max"],
            "Fwd Packet Length Min": data["fwd_pkt_len_min"],
            "Fwd Packet Length Mean": data["fwd_pkt_len_mean"],
            "Fwd Packet Length Std": data["fwd_pkt_len_std"],
            "Bwd Packet Length Max": data["bwd_pkt_len_max"],
            "Bwd Packet Length Min": data["bwd_pkt_len_min"],
            "Bwd Packet Length Mean": data["bwd_pkt_len_mean"],
            "Bwd Packet Length Std": data["bwd_pkt_len_std"],
            "Flow Packets/s": data["flow_pkts_s"],
            "Bwd IAT Total": data["bwd_iat_tot"],
            "Bwd IAT Mean": data["bwd_iat_mean"],
            "Bwd IAT Std": data["bwd_iat_std"],
            "Bwd IAT Max": data["bwd_iat_max"],
            "Fwd PSH Flags": data["fwd_psh_flags"],
            "Min Packet Length": data["pkt_len_min"],
            "Max Packet Length": data["pkt_len_max"],
            "Packet Length Mean": data["pkt_len_mean"],
            "Packet Length Std": data["pkt_len_std"],
            "Packet Length Variance": data["pkt_len_var"],
            "SYN Flag Count": data["syn_flag_cnt"],
            "PSH Flag Count": data["psh_flag_cnt"],
            "URG Flag Count": data["urg_flag_cnt"],
            "Down/Up Ratio": data["down_up_ratio"],
            "Average Packet Size": data["pkt_size_avg"],
            "Avg Fwd Segment Size": data["fwd_seg_size_avg"],
            "Avg Bwd Segment Size": data["bwd_seg_size_avg"],
            "Subflow Fwd Bytes": data["subflow_fwd_byts"],
            "min_seg_size_forward": data["fwd_seg_size_min"],
        }
        data.update(cicids_mapping)
    ...
    def update_active_idle(self, current_time: float):
        """Adds a packet to the current list of packets.

        Args:
            current_time: Current time to update active/idle
        """
