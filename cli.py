...
def flow_ids(
    ...
    enable_packet_drop: bool = typer.Option(
        True,
        "--enable-packet-drop/--disable-packet-drop",
        help="Enable/disable packet dropping for malicious flows (realtime mode only, default: enabled)",
    ),
) -> None:
    """
    ...
    try:
        ...
        if pcap:
            ...
            # Initialize packet dropper for dropping malicious packets (only for realtime mode)
            packet_dropper = PacketDropper(enable_firewall_block=False) if enable_packet_drop else None
            ...
            def on_flow_complete(flow_data: Dict[str, Any]) -> None:
                ...
                try:
                    ...
                    if sigma_hits:
                        ...
                        # Drop malicious flow if action is "drop" and packet dropping is enabled
                        if action_val == "drop" and packet_dropper:
                            try:
                                src_ip = ev.get("src_ip", "")
                                src_port = ev.get("src_port", 0)
                                dst_ip = ev.get("dst_ip", "")
                    ...
                    # If Sigma didn't match, but ML detected anomaly
                        ...
                        # Drop malicious flow (if packet dropping is enabled)
                        if packet_dropper:
                            try:
                                src_ip = ev.get("src_ip", "")
                                src_port = ev.get("src_port", 0)
            ...
            capture_and_detect_realtime(
                ...
                packet_dropper=packet_dropper,
            )
            
            # Log drop statistics (if packet dropping was enabled)
            if packet_dropper:
                try:
                    stats = packet_dropper.get_drop_stats()
                    log_action(f"cli.flow_ids:drop_stats flows_dropped={stats['dropped_flows_count']} packets_dropped={stats['total_packets_dropped']}")
                except Exception as e:
                    log_error(f"cli.flow_ids:error getting drop stats {e}")
